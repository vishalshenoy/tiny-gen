...
app = modal.App(name="tinygen-app", image=image,
                secrets=[modal.Secret.from_name("tinygen-secret")])
...
@fastapi_app.post("/generate-diff", response_model=DiffResponse)
...
@app.function(image=image, secrets=[modal.Secret.from_name("tinygen-secret")])
@modal.asgi_app()
